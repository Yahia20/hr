...
- `document_history` — one row per date change to a document (recorded by the documents PATCH handler): document_id, old/new start & end dates, changed_by, changed_at. Preserves the renewal audit trail; served by `GET /api/documents/{id}/history`
